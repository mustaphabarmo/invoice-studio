...
function HistoryPage() {
  ...
  useEffect(() => { load(); window.addEventListener("kumail-data-change", load); return () => window.removeEventListener("kumail-data-change", load); }, []);
  ...
  function remove(invoice: SavedInvoice) { if (window.confirm(`Delete invoice #${invoice.data.invoiceNumber}?`)) { deleteInvoice(invoice.id); if (selected?.id === invoice.id) setSelected(undefined); } }
