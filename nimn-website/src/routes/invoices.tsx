...
function InvoicePage() {
  ...
  async function persist() { try { const saved = await saveInvoice(data, "Final", savedId); setSavedId(saved.id); setNotice("Invoice saved to history"); } catch(e) { setNotice(e instanceof Error ? e.message : "Could not save invoice"); } setTimeout(() => setNotice(""), 2400); }
